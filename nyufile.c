...
void option_rR(unsigned char command, unsigned char* diskImage, unsigned char* fileName, unsigned char* shaHash, int sValid){
    ...
    // non contiguous 
    //else{
        // assume 100 entry matches 
        //DirEntry** entries = malloc(sizeof(DirEntry*)*101);
        //findDeletedFiles(disk, filee, rootDirStartIndex, bytesPerCluster, reservedArea, 
        //fatArea, rootClusterIndex, bytesPerFAT, (unsigned int) numOfFATS, 0, shaHash, 
        //entries, rootDirStartIndex);
    //}
    return;
}
void searchDeletedFiles(unsigned char* disk, unsigned char* fileName, unsigned int rootDirStartIndex, 
...
, unsigned int preserveStartDataSect){
    // declare functions
    unsigned int getNextAvailableSpace(DirEntry** entryz);
    void searchContiguous(unsigned char* disk, unsigned char* shaHash, DirEntry** entryArray, 
    unsigned int numOfEntries, unsigned int bytesPerClus, unsigned int startDataSect);
    
    // changing variables  
    unsigned int matchCount = foundcount;
    DirEntry** nameMatchEntries = entries;
    unsigned int startDataSect = preserveStartDataSect;

    // search for name match of deleted files
    for (unsigned int i = 0; i < bytesPerClus/32 ; i++){
        DirEntry* rootEntry = (DirEntry*) &disk[rootDirStartIndex+(i*32)];
        unsigned char* dirName = rootEntry->DIR_Name;
        // BASE CASE 1 - if no more files in directory
        if(!dirName[0]){
            ...
            for(unsigned int j = 1; j < 9; j++){
                if(fileName[j]){
                    if(fileName[j] == '/'){
                        ...
                        if(fileName[j+1]){
                            compareFileName[8] = fileName[j+1];
                        }
                        if(fileName[j+2]){
                            compareFileName[9] = fileName[j+2];
                        }
                        if(fileName[j+3]){
                            compareFileName[10] = fileName[j+3];
                        }
                        break;
                    }
                    else{
            ...
            int matches1 = TRUE;
            // check if filename from index 1 to end matches given name to recover
            for(unsigned int j = 1; j < 11; j++){
                if (compareFileName[j] == dirName[j]){
                    continue;
                }
            ...
            // FOUND - matches name
            unsigned int index = getNextAvailableSpace(nameMatchEntries);
            // add to DirEntry array 
            nameMatchEntries[index] = rootEntry;
            // add found count
            matchCount+=1;
        }
        else{
    ...
    if((rootDirFatLoc < 0x0ffffff8)&&(rootDirFatLoc != 0x0ffffff7)&&(rootDirFatLoc != 0x00000000)){
        unsigned int newOffset = (rootDirFatLoc-2) * bytesPerClus;
        unsigned int newRootStartIndex = fatAreaStartIndex+fatAreaSize+newOffset;
        findDeletedFiles(disk, fileName, newRootStartIndex, bytesPerClus, fatAreaStartIndex, 
        fatAreaSize, rootDirFatLoc, bytesPerFat, numOfFats, matchCount, shaHash, 
        nameMatchEntries, startDataSect);
        return;
    }
    // AT THIS POINT, all entries with matching name have been added to array
    searchContiguous(disk, shaHash, nameMatchEntries, matchCount, bytesPerClus, startDataSect);
    return;
}
// search through each entry
void searchContiguous(unsigned char* disk, unsigned char* shaHash, DirEntry** entryArray, unsigned int numOfEntries, 
unsigned int bytesPerClus, unsigned int startDataSect){
    // declare functions
    void recursiveConstructChain(unsigned int* possibleIndices, unsigned int n, unsigned int r, unsigned char* disk
    , unsigned char* shaHash, unsigned int bytesPerClus, unsigned int startDataSect)
    int SHA1_Init(SHA_CTX *c);

    // for each entry in entryArray
    for(int i = 0; i < numOfEntries; i ++){
        // entry and file size of current iteration
        DirEntry* entry = entryArray[i];
        unsigned int entrySize = entry->DIR_FileSize; // NEED
        unsigned int fileClus = entry->DIR_FstClusHI*(pow(2,16))+entry->DIR_FstClusLO;
        unsigned int clusterSpanCount = entrySize/bytesPerClus; // NEED
        if(entrySize%bytesPerClus != 0){
            clusterSpanCount+=1;
        }
        // construct possible indices array
        unsigned int possibleIndices[20];
        ...
        for(unsigned int j = 2; j < 22; j++){
            if(j == fileClus){
                ...
                possibleIndices[index] = j;
                index++;
            }
        }
        unsigned int r = clusterSpanCount-1;
        unsigned int n = sizeof(possibleIndices)/sizeof(possibleIndices[0]);
        int found = recursiveConstructChain(possibleIndices, n, r, disk, shaHash, 
        entry, bytesPerClus, startDataSect);
    ...
    // store non conv hash
    unsigned char nonConvHash[20]; 
    SHA1_Final(nonConvHash, &context);     
    SHA1(cmpContent, sizeOfFile, nonConvHash);
    int SHA1_Final(unsigned char *md, SHA_CTX *c);
}
int recursiveConstructChain(unsigned int* possibleIndices, unsigned int n, unsigned int r){
    int findCombinations(unsigned int* possibleIndices, unsigned int n, unsigned int r, unsigned int index, unsigned int* chain, unsigned int i);
    unsigned int chain[r];
    int found = findCombinations(possibleIndices, n, r, 0, chain, 0);
}
int findCombinations(unsigned int* possibleIndices, unsigned int n, unsigned int r, unsigned int index, unsigned int* chain, unsigned int i){
    int found = FALSE;
    // BASE CASE 
    if(index == r){
        // CHAIN has a combo --> check sha
    }

    // BASE CASE - no more elements to check or the correct sha has been found
    if (i >= n || found == TRUE){
        return found;
    }

    chain[index] = possibleIndices[i];
    findCombinations(possibleIndices, n, r, index+1, chain, i+1);
    findCombinations(possibleIndices, n, r, index, chain, i+1);
}

void recoverNonContFiles(){
...
// HELPERS
unsigned int getNextAvailableSpace(DirEntry** entryz){
    unsigned int nextIndexAvailable;
...
// https://stackoverflow.com/questions/80657/makefiles-cflags
// https://stackoverflow.com/questions/3602927/add-compiler-option-without-editing-makefile
// https://gcc.gnu.org/onlinedocs/gcc-4.8.0/gcc/C-Dialect-Options.html
// https://stackoverflow.com/questions/13400605/how-can-i-avoid-gcc-warning-for-plain-char-to-unsigned-char-or-signed-cha
// https://stackoverflow.com/questions/62096126/c-segmentation-fault-printing-an-unsigned-char
// https://www.tutorialspoint.com/cprogramming/c_data_types.htm
// https://stackoverflow.com/questions/13225538/casting-a-char-valued-255-leads-to-an-unsigned-int-value-of-4294967295-what-giv
// https://stackoverflow.com/questions/2699534/how-to-convert-int-to-int
// https://jameshfisher.com/2017/01/28/mmap-file-write/
// https://linux.die.net/man/3/sha1
...
// discord message: 
//  - user: Charlie_Xiao
//  - group: CS202 discord channel
//  - message: "One way to do that is to use the %02x specifier for each unsigned char given by SHA1(), 
//              then compare the corresponding two digits in the given argument."
